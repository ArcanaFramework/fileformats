...
class BaseDirectory(FileSet):
    ...
    def set_fspaths(self, fspaths: ty.List[Path]):
        ...
        matches = [p for p in fspaths if Path(p).is_dir() and self.contents_match(p)]
        ...
        self.exists = True
        self.fspath = absolute_path(matches[0])
    ...
    def all_file_paths(self):
        ...
        return chain(
            *(
                ...
                for root, _, files in os.walk(self.fspath)
            )
        )

    def copy_to(self, fspath: str, symlink: bool = False):
        ...
        copy_dir(self.fspath, fspath)
        cpy = copy(self)
        cpy.set_fspaths([fspath])
        return cpy
