...
class CifsMountIndentifier:
    ...
    def on_cifs(cls, fname: Path) -> bool:
        ...
        for fspath, fstype in cls.mount_table:
            if str(fname).startswith(fspath):
                return fstype
        return False

    @classmethod
    def generate_cifs_table(cls) -> list[str]:
        """
    ...
    @classmethod
    def parse_mount_table(cls, exit_code, output) -> list[str]:
        """
    ...
    @classmethod
    @contextmanager
    def patch_table(cls, mount_table):
